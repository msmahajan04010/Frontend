...
<div className="relative z-10 py-12 sm:py-20 px-0">
  <div className="max-w-full mx-auto">
          ...
          >
            ...
            <div className="relative h-full flex flex-col md:flex-row items-center justify-between p-8 md:p-12">
              <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left mb-8 md:mb-0">
                <div className={`w-24 h-24 rounded-full bg-gradient-to-br ${slide.color} flex items-center justify-center mb-6 transform transition-transform duration-500 hover:scale-110 hover:rotate-12`}>
                  <slide.icon className="w-12 h-12 text-white" />
                </div>
                <h2 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">{slide.title}</h2>
                <p className="text-lg text-gray-600 max-w-xl mb-6">{slide.description}</p>
                <button className={`px-8 py-3 bg-gradient-to-r ${slide.color} text-white rounded-full font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300`}>
                  Learn More
                </button>
              </div>
              <div className="flex-1 flex items-center justify-center">
                <div className="grid grid-cols-2 gap-4 w-full max-w-md">
                  {slide.features.map((feature, idx) => (
                    <div
                      key={idx}
                      className="bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transform hover:-translate-y-2 transition-all duration-300 border-2 border-gray-100"
                      style={{ animation: `fadeInUp 0.6s ease-out ${idx * 0.1}s both` }}
                    >
                      <div className={`w-3 h-3 rounded-full bg-gradient-to-r ${slide.color} mb-3`}></div>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3 z-10">
        {slides.map((_, index) => (
          <button
            ...
            className={`transition-all duration-300 rounded-full ${index === activeSlide ? 'w-12 h-3 bg-gradient-to-r from-blue-600 to-purple-600' : 'w-3 h-3 bg-gray-400 hover:bg-gray-600'}`}
          />
        ))}
      </div>
    </div>
    ...
    <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-4 mt-8 px-4 sm:px-6">
      {slides.map((slide, index) => {
        ...
        return (
          <div
            ...
            className={`cursor-pointer rounded-xl p-6 transition-all duration-300 transform hover:-translate-y-2 hover:shadow-xl ${index === activeSlide ? `bg-gradient-to-br ${slide.color} text-white shadow-lg scale-105` : 'bg-white text-gray-800 hover:bg-gray-50'}`}
          >
            <SlideIcon className={`w-8 h-8 mb-3 ${index === activeSlide ? 'text-white' : 'text-gray-600'}`} />
            <h3 className="font-bold text-lg mb-2">{slide.title}</h3>
            <p className={`text-sm ${index === activeSlide ? 'text-white opacity-90' : 'text-gray-600'}`}>Hover to explore</p>
          </div>
        );
      })}
    </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
            {values.map((value, index) => (
              <div key={index} className="group text-center">
                <div className="bg-gradient-to-r from-cyan-500 to-blue-500 w-16 sm:w-20 h-16 sm:h-20 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg">
